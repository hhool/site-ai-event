# Operations Index

This page is the entry point for all operations runbooks and templates.

## Incident Response

- [On-Call Playbook](oncall-playbook.md)
- [On-Call Handover Template](oncall-handover-template.md)
- [Postmortem Template](postmortem-template.md)

## Release Operations

- [Release Runbook](../release/release-runbook.md)
- [Post-Release 24h Observation Checklist](post-release-24h-checklist.md)

## Reporting Templates

- [Weekly Operations Report Template](weekly-ops-report-template.md)
- [Monthly Stability Report Template](monthly-stability-report-template.md)

## Process Governance

- [Operations Process Plan](operations-process-plan.md)

## Governance References

- [Branch Protection Guide](../governance/branch-protection.md)
- [Rulesets Setup Guide](../governance/rulesets-setup.md)
- [Repository Settings Baseline](../governance/repository-settings-baseline.md)

## Suggested Operating Rhythm

1. Daily: monitor alerts and update incidents using On-Call Playbook.
2. Shift handover: use the On-Call Handover Template.
3. After incidents: complete a Postmortem Template.
4. After each release: run the Post-Release 24h checklist.
5. Weekly and monthly: publish weekly and monthly reports.
6. Keep process ownership and cadence aligned with the Operations Process Plan.
